        ...
        {!isCompleted && (
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button cursor-target" {...nextButtonProps}>
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator cursor-target" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'var(--border)', color: 'var(--text-secondary)' },
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = {
    incomplete: { width: 0, backgroundColor: 'var(--border)' },
